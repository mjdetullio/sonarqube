...
public class MigrateQualityGatesConditions extends BaseDataChange {
  ...
  public void execute(Context context) throws SQLException {
    ...
    Set<String> qualityGatesUpdated = new LinkedHashSet<>();
    update.execute(new MigrationHandler(qualityGatesUpdated, system2.now()));
    if (!qualityGatesUpdated.isEmpty()) {
      LOG.warn("The following Quality Gates have been updated to compare with the leak period: {}.", Joiner.on(", ").join(qualityGatesUpdated));
    }
  }

  private static class MigrationHandler implements MassUpdate.Handler {
    private final Date now;
    private final Set<String> qualityGatesUpdated;

    public MigrationHandler(Set<String> qualityGatesUpdated, long now) {
      this.qualityGatesUpdated = qualityGatesUpdated;
    ...
    public boolean handle(Select.Row row, SqlStatement update) throws SQLException {
      ...
      qualityGatesUpdated.add(row.getString(2));
      return true;
    }
  }
}
